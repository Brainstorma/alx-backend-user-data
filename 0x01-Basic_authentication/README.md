...
This project involves learning about the authentication process and implementing a Basic Authentication on a simple API written in Python Flask Framework. You will need to have some knowledge in:
...
- Clone this repository: `git clone https://github.com/Brainstorma/alx-backend-user-data.git`
- Change directory to the project folder: `cd alx-backend-user-data/0x01-Basic_authentication`
- Install the dependencies: `pip install -r requirements.txt`
- Run the API server: `python3 api/v1/app.py`

## Tasks

The project consists of 14 tasks, each with a corresponding Python file and a test file. You can find the links to the tasks below:

- [0. Simple API](^1^): Create a simple API with two routes: `/` and `/users`.
- [1. Error handler: Unauthorized](^2^): Implement an error handler for the status code 401 (Unauthorized).
- [2. Error handler: Forbidden](^3^): Implement an error handler for the status code 403 (Forbidden).
- [3. Auth class](^4^): Create a class to manage the API authentication.
- [4. Define which routes don't need authentication](^5^): Define a method in the Auth class that returns `False` if the path is one of the public routes.
- [5. Request validation!](^6^): Override the `Flask.request` object to include a `current_user` instance attribute.
- [6. Basic auth](^7^): Create a class that inherits from `Auth` and implements the `Basic` authentication mechanism.
- [7. Basic - Base64 part](^8^): Implement a method that extracts the Base64 part of the Authorization header for a Basic Authentication.
- [8. Basic - Base64 decode](^9^): Implement a method that decodes a base64-encoded authorization header.
- [9. Basic - User credentials](^10^): Implement a method that extracts user credentials from a base64-decoded authorization header that uses the Basic authentication flow.
- [10. Basic - User object]: Implement a method that retrieves a user based on the user's authentication credentials.
- [11. Basic - Overload current_user - and BOOM!]: Override the `current_user` method of the `BasicAuth` class to return the current user instance based on the authorization header.
- [12. Basic - Allow password with ":"]: Modify the regular expression used to extract user credentials to allow passwords with `:`.
- [13. Require auth with stars]: Modify the `require_auth` method of the `Auth` class to allow routes that start with any of the public paths.
