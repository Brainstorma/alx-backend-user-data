...
This project is about handling personal data securely, such as obfuscating Personally Identifiable Information (PII) fields, hashing passwords, and authenticating with a secure database.

## Learning objectives

By the end of this project, you are expected to be able to:

- Understand the concept of PII and why it is important to protect it
- Implement a log filter that will obfuscate PII fields
- Encrypt passwords and check them for validity
- Create and manage a MySQL database using SQLAlchemy
- Create a user table in the database and store hashed passwords
- Create a basic authentication system that requires username and password
...
- bcrypt 3.2 or higher

## Environment variables

The following environment variables are needed to run the project:

- `PERSONAL_DATA_DB_USERNAME`: the username for the MySQL database (default: "root")
- `PERSONAL_DATA_DB_PASSWORD`: the password for the MySQL database (default: "")
- `PERSONAL_DATA_DB_HOST`: the host for the MySQL database (default: "localhost")
- `PERSONAL_DATA_DB_NAME`: the name of the database to use (default: "user_data")
...
Write a function that returns the log message obfuscated: One regex per line and use parenthesis for “or”. The regex must not contain square brackets

### [1. Log formatter](^2^)

Write a function that returns a log message obfuscated: Use a regex to replace occurrences of certain field names (example: "password", "date_of_birth") with "[REDACTED]".

### [2. Create logger](^3^)

Write a function that takes no arguments and returns a logging.Logger object.

### [3. Connect to secure database](^4^)

Write a function that returns a connector to the database (mysql.connector.connection.MySQLConnection object).

### [4. Read and filter data](^5^)

Write a function that obtains a database connection using get_db and queries the users table.

### [5. Encrypting passwords](^6^)

Write a function that expects one string argument name password and returns a salted, hashed password, which is a byte string.

### [6. Check valid password](^7^)

Write a function that expects 2 arguments and returns a boolean.
